...
function selectEventAjax(){
	...
	console.log("selectEventAjax data => ", data);
	...
	$.ajax({
		...
		success : function(res){
			...
			console.log("selectEventAjax data res => ", res);
...
// @url		    : /hdofc/event/selectEventDetail
// @author      : 정기쁨
// @date        : 2024.09.23
// @jsp         : hdofc/event/selectEventDetail
// @description : 쿠폰 발급 리스트 조회
	...
	$.ajax({
		...
		success: function(res) {
			...
			if(res.total > 0){
				...
				res.couponList.forEach( (couponVO,index) =>{
					mbrNm = couponVO.mbrNm;
					mbrId = couponVO.mbrId;
					frcsNm = couponVO.bzentNm ? couponVO.bzentNm : '-';
